...
import { MailOpen, Plus, Search, Edit, Trash2, Printer, FileText, X } from 'lucide-react';
...
export default function SuratMasukPage() {
  ...
  const fetchLetters = () => {
    ...
    fetch('/api/surat?tipe=masuk')
      ...
      .catch((err) => toast.error('Gagal memuat daftar surat masuk.'))
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Manajemen arsip surat masuk yang diterima oleh Komite Tenaga Kesehatan Lain RSUD OKU TIMUR.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead className="bg-slate-950 text-slate-300 uppercase font-bold text-[10px] tracking-wider border-b border-slate-800">
              <tr>
                ...
                <th className="px-5 py-4">Yth. Tujuan</th>
            ...
            <div className="p-5 border-b border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-100 flex items-center gap-2">
                  <MailOpen className="w-5 h-5 text-emerald-400" />
                  {editingLetter ? 'Edit Surat Masuk' : 'Buat Surat Masuk Baru'}
                </h3>
              </div>
              <button
