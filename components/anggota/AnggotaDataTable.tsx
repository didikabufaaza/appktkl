    ...
    () => [
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          return (
            <div className="text-xs text-emerald-400 font-mono flex items-center gap-1.5 whitespace-nowrap">
      ...
      },
      {
        ...
        cell: (info) => {
          const url = String(info.getValue() || info.row.original.sipUrl || info.row.original.strUrl || '').trim();
          if (!url) return <span className="text-slate-500 text-xs italic">Kosong</span>;
          return (
            <button
              onClick={() => setPreviewDoc({ title: `Berkas - ${info.row.original.namaLengkap}`, url })}
              className="inline-flex items-center gap-1 text-xs text-teal-400 hover:text-teal-300 font-semibold underline underline-offset-4"
            >
        ...
        cell: (info) => {
          ...
          return (
            <div className="flex items-center justify-end gap-2">
              ...
              {permissions?.canEditMemberData !== false && onEdit && (
                <button
              ...
              {permissions?.canDeleteMemberData !== false && onDelete && (
                <button
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-sm">
        ...
        {/* Action Buttons Scoped by Permissions */}
        <div className="flex items-center gap-2">
          ...
          {permissions?.canRegisterNew !== false && (
            <Link
              href="/anggota/baru"
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-bold text-xs shadow-md shadow-emerald-500/20 transition-all"
            >
